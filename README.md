...
The model picker auto-detects Codex, Claude, and OpenCode when they are installed locally. Codex and Claude have realtime percentage adapters. OpenCode exposes token and cost statistics rather than a percentage limit, so it is listed as detected but hidden by default instead of showing a misleading percentage bar. Additional providers can be added by giving the bridge a realtime usage adapter and adding one entry to the model registry.
